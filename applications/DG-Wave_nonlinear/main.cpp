...
public:
    ...
    void operator()(const double t, const Geometry::PointPhysical& p, LinearAlgebra::NumericalVector& ret) {
        ...
        for (int i = 1; i < n_ + 1; ++i) {
            ret[1] += B_[i] * std::sin(i * k_ * x) * std::cosh(i * k_ * p[DIM - 1]) / std::cosh(i * k_);
        }
    }
...
public:
    waveMakerFileReader(){
        ...
        while (!position.eof()) {
            position >> time;position>>s;
            assert(!derivative.eof());
            derivative>>temp;derivative>>u;
            assert(time==temp);
            t.push_back(time);
            x.push_back(s);
            dx.push_back(u);
            assert(!secondDerivative.eof());
            secondDerivative>>temp;secondDerivative>>u;
            assert(time==temp);
            ddx.push_back(u);
        }
...
public:
    ...
    bool virtual initialise() {
        ...
        description.bottomLeft_[DIM - 1] = 1;
        ...
        addMesh(description, Base::RECTANGULAR, 0, 0, 2, 0); //keeps original node z-coordinates+construce surface matrices that dont have to account for surface tilt
    ...
    static double waveMakerPosition() {
        ...
        //return (1. - cos(2. * M_PI * t / 3.)) / 1000.;
        //return 0;
    }

    static double waveMakerDerivative() {
        return piston(t).second;
        //return sin(2. * M_PI * t / 3.) / 1000. * 2. * M_PI / 3.;
        //return 0;
    }
    ...
    public:
        ...
        void elementIntegrand(const Base::Element* element, const Geometry::PointReference& p, LinearAlgebra::Matrix& ret) {
            ...
            phiDerivI.resize(DIM);phiDeriv.resize(DIM);
    ...
    public:
        void faceIntegrand(const Base::Face* face, const LinearAlgebra::NumericalVector& normal, const Geometry::PointReference& p, LinearAlgebra::Matrix& ret){
            const LinearAlgebra::Matrix& coefficients=face->getPtrElementLeft()->getTimeLevelData(0);
            ...
            //std::cout<<Base::L2Norm(normal)<<" "<<sqrt(1+n_/L*n_/L*(coefficients(0,3)-coefficients(0,2))*(coefficients(0,3)-coefficients(0,2)))*L/n_/2.<<std::endl;
            for(int i=0;i<n;++i){
                for(int j:faceNodes){//use linearity of the basisfunctions and a taylor approximation 
                    for(int k:faceNodes){
                        for(int l=0;l<n;++l){//the matrix tester needs the transpose
                            ret(j,i)+=face->basisFunction(i,p)*face->basisFunction(l,p)*coefficients(0,l)*
                                    n_/L/2.*(2*(j%2)-1)*coefficients(0,k)*(2*(k%2)-1)/
                                    sqrt(1+n_*n_/L/L*(coefficients(0,3)-coefficients(0,2))*(coefficients(0,3)-coefficients(0,2)))
                                    /Base::L2Norm(normal);
                        }
                    }
                }
        ...
        void faceIntegrand(const Base::Face* face, const LinearAlgebra::NumericalVector& normal, const Geometry::PointReference& p, LinearAlgebra::Matrix& ret) {
            ...
            for (int i = 0; i < n; ++i) {
                ...
                for (int k : faceNodes) {
                    ...
                    for (int j = 0; j < n; ++j) {
                        if (physicalNode[0] < halfL) {//does not cause any magic phenomena at x=halfL
                            double x = (halfL - physicalNode[0]) / (halfL - waveMakerPosition());
                            ret(i, j) += face->basisFunction(j, p) * x * face->basisFunction(k, p) * face->basisFunctionDeriv(i, 0, p) * waveMakerDerivative() / Base::L2Norm(normal);
                        }
                    }
                }
            }
    ...
    public:
        ...
        void faceIntegrand(const Base::Face* face, const LinearAlgebra::NumericalVector& normal, const Geometry::PointReference& p, LinearAlgebra::Matrix& ret) {
            ...
            if (face->getFaceType() == Geometry::WALL_BC && std::fabs(pPhys[0] - waveMakerPosition()) < 1e-9) {
                ...
                for (int i = 0; i < n; ++i) {
                    for (int j : nodes) {
                        ret(j, i) =  waveMakerDerivative() * face->basisFunction(i, p) / Base::L2Norm(normal) * face->basisFunctionDeriv(j, DIM - 1, p);
                    }
                }
            } else {
                int n = face->getNrOfBasisFunctions();
    ...
    void writeToTecplotFile(const ElementT* element, const PointReferenceT& p, std::ostream& out) {
        ...
        out << value[0] << " " << value[1] + t * 0.6;//correct by the average water height to prevent drifting
        ...
        for (Base::Face* face : meshes_[0]->getFacesList()) {
            if (face->getFaceType() == Geometry::OPEN_BC) {
                ...
                for (int i = 0; i < n; ++i) {
                    ...
                    initialconditions(0, i + n) = functionvalue[0] / n;
                    initialconditions(1, i + n) = functionvalue[1] / n;
    ...
    void constructC() {
        ...
        for (Base::Element* element : meshes_[0]->getElementsList()) {
            ...
            for (int i = 0; i < element->getNrOfBasisFunctions(); ++i) {
                missingMultiplier(i, i) = 1 - meshes_[1]->getNodes()[element->getPhysicalGeometry()->getNodeIndex(i)][DIM - 1];
            }
        ...
        for (Base::Face* face : meshes_[0]->getFacesList()) {
            ...
            if (face->getFaceType() == Geometry::WALL_BC) {
                ...
                for (int i = 0; i < left->getNrOfBasisFunctions(); ++i) {
                    missingMultiplier(i, i) = 1 - meshes_[1]->getNodes()[left->getPhysicalGeometry()->getNodeIndex(i)][DIM - 1];
                }
                result = result*missingMultiplier;
            } else {
                result *= 0;
    ...
    void moveNodes() {
        ...
        referenceFirstNode = &meshes_[1]->getNodes()[0];
            ...
            if (face->getFaceType() == Geometry::OPEN_BC) {
                ...
                for (int i = 0; i < faceNodes.size(); ++i) {
                    ...
                    for (Geometry::PointPhysical* point : nodesBelowASurfaceNode_[faceNodes[i]]) {
                        ...
                        (*point)[DIM - 1] = (1. - zOld)*(solution[0])-0.6;
                        //(*point)[DIM-1]=zOld+(zOld)*solution[0];
                    }
                }
            }
        }
        ...
        double rNew(waveMakerPosition()), halfL(L);
        for (const Geometry::PointPhysical& node : meshes_[0]->getNodes()) {
            if ((node)[0] < halfL) {
                const_cast<Geometry::PointPhysical&> (node)[0] = (halfL - rNew) / (halfL - rOld)*((node)[0] - rOld) + rNew;
            }
        }
    ...
    bool solveStormerVerlet() {
        ...
        VecCopy(etaActually, surfaceRHS);
        KSPSetOperators(surface, surfaceMass, surfaceMass, SAME_NONZERO_PATTERN);
        //KSPSolve(surface,surfaceRHS,etaActually);
        //KSPGetConvergedReason(surface,&conferge);
        //KSPGetIterationNumber(surface,&iterations);
        //std::cout<<"Finalizing interpolation (1): KSP solver ended because of "<<KSPConvergedReasons[conferge]<<" in "<<iterations<<" iterations."<<std::endl;
        ...
        KSPSetOperators(interior, BigMatrix, BigMatrix, DIFFERENT_NONZERO_PATTERN);
            ...
            for (int j = 0; j < numberOfTimeSteps; ++j) {
                ...
                while (normInterior + normSurface > accuracy) {
                    ...
                    if(first)
                        ...
                        KSPSetOperators(interior, BigMatrix, BigMatrix, SAME_NONZERO_PATTERN);
                ...
                while (normInterior + normSurface > accuracy) {
                    ...
                    MatAXPY(BigMatrix, dt / 2., C, DIFFERENT_NONZERO_PATTERN);
                    MatTranspose(BigMatrix, MAT_REUSE_MATRIX, &BigMatrix);
    ...
    void testMatrices() {
        ...
        KSPSetOperators(laplace, S, S, DIFFERENT_NONZERO_PATTERN);
