...
BraggStackBandstructure::BraggStackBandstructure(double eps1, double eps2, double fraction)
    : eps1_ (eps1)
    , eps2_ (eps2)
    , fraction_ (fraction)
{}
        ...
        LinearAlgebra::SmallVector<3> kpoint, double omegaMax) const
{
    for(std::size_t i = 0; i < 3; ++i)
        logger.assert_always(std::abs(kpoint[i]) <= (M_PI+1e-8), "Only implemented for k in first Brillouin zone.");
...
double bisect(std::function<double(double)> function, double left, double right, double tol = 1e-8)
{
...
void BraggStackBandstructure::findRoots(LinearAlgebra::SmallVector<3> k, double omegamax, bool tm, std::vector<double> &out) const
{
...
double BraggStackBandstructure::findRoot(LinearAlgebra::SmallVector<3> k, std::size_t n, bool tm) const
{
...
void BraggStackBandstructure::findRootsInterval(LinearAlgebra::SmallVector<3> k, double omin, double omax, bool tm,
                                                std::vector<double> &out) const
{
    ...
    {
        // Left endpoint is small enough to expect there to be a root very nearby
        // either due to finite precision in computing f, or in the point omin.
        // We do not do the same check for fval2 to prevent duplicate detection.
    ...
    else if(std::abs(fval2) > 1e-8 && fval1 * fval2 < 0)
    {
        ...
        std::function<double(double)> func = [=] (double o) { return tm ? valuetm(o, k) : valuete(o, k);};
    ...
    else if (std::abs(fval2) < 0.5 && std::abs(fval1) < 0.5)
    {
        ...
        // multiply the values of valuete/valuetm by the original sigh of fval, this ensures
        // that we are always having positive values and are looking for a minimum.
        double sign = fval1 > 0 ? 1 : -1;
        std::function<double(double)> func = [=] (double o) { return sign * (tm ? valuetm(o, k) : valuete(o, k));};
        ...
        // Two internal points that are used to determine how to shrink the interval
        double c = b - (b-a)/phi;
        double d = a + (b-a)/phi;
...
std::unique_ptr<typename BandStructure<3>::LineSet> BraggStackBandstructure::computeLines(
        LinearAlgebra::SmallVector<3> point1, LinearAlgebra::SmallVector<3> point2, double maxFrequency) const
{
    ...
    for(std::size_t i = 0; i < 3; ++i)
    {
        logger.assert_always(std::abs(point1[i]) <= (M_PI + 1e-8), "Only implemented for k in first Brillouin zone.");
        logger.assert_always(std::abs(point2[i]) <= (M_PI + 1e-8), "Only implemented for k in first Brillouin zone.");
    }
    ...
    std::map<LinearAlgebra::SmallVector<2>, std::pair<std::size_t, std::size_t>> modes2;
    ...
    while (!queue.empty())
    {
        ...
        double tresh = std::cosh(std::sqrt(k1z)*fraction_ + std::sqrt(k2z) * (1-fraction_));
        ...
        if(!added)
        {
            // It is a new wavevector/reciprocal lattice point. We have to determine how many
            // modes have to be included in the banddiagram, or at least an upper bound to
            // this. For this we take the maximum of the amount of bands at either end of
            // the line.
            modes[newMode] = 1; // Single degeneracy
            std::vector<double> roots;
            // Maybe kr = kp - point1?
            LinearAlgebra::SmallVector<3> kr ({point1[0], kp[0]-point1[1], kp[1]-point1[2]});
            ...
            roots.clear();
            kr = LinearAlgebra::SmallVector<3>({point2[0], kp[0]-point2[1], kp[1] - point2[2]});
            findRoots(kr, maxFrequency, false, roots);
    ...
    // Compute line set.
    std::unique_ptr<BraggStackBandstructure::LineSet> result (new LineSet(*this, l2, point1[0], point2[0]));
    for(auto& mode : modes)
    {
...
double BraggStackBandstructure::valuete(double omega, LinearAlgebra::SmallVector<3> k) const
{
    // See Yarev & Yeh 6.2-24 for the original formula
    // With the difference that we have the repetition in X instead of Z direction
    double kx2 = k[1] * k[1];
    double ky2 = k[2] * k[2];
    std::complex<double> k1z = std::sqrt(std::complex<double>(eps1_ * omega * omega - kx2 - ky2));
    std::complex<double> k2z = std::sqrt(std::complex<double>(eps2_ * omega * omega - kx2 - ky2));
    ...
    double ad2 = (std::cos(k1z * fraction_) * std::cos(k2z * b)
                - 0.5  * kk * std::sin(k1z * fraction_) * std::sin(k2z*b)).real();
    return std::cos(k[0]) - ad2;
}

double BraggStackBandstructure::valuetm(double omega, LinearAlgebra::SmallVector<3> k) const
{
    ...
    std::complex<double> k1z = std::sqrt(std::complex<double>(eps1_ * omega * omega - kx2 - ky2));
    std::complex<double> k2z = std::sqrt(std::complex<double>(eps2_ * omega * omega - kx2 - ky2));
    std::complex<double> kk = (eps2_ * k1z) / (eps1_ * k2z) + (eps1_ * k2z) / (eps2_ * k1z);
    if(omega < 1e-6 && kx2 + ky2 < 1e-6) // TODO: Better bounds
    {
        kk = std::sqrt(eps1_ / eps2_ ) + std::sqrt(eps2_ / eps1_);
    }
...
// Lineset
BraggStackBandstructure::LineSet::LineSet(const BraggStackBandstructure &structure, double l, double x1, double x2)
...
double BraggStackBandstructure::LineSet::frequency(std::size_t line, double interpolation) const
{
    logger.assert_debug(line >= 0 && line <= numberOfLines(), "Invalid line number");
    // Find line number
...
void BraggStackBandstructure::LineSet::addLine(
        std::size_t multiplicity, double x, double y, std::pair<std::size_t, std::size_t> modes)
{
    multiModes_.emplace_back(x, y, modes.first, modes.second, multiplicity);
    totalNumberOfLines_ += modes.first + modes.second;
}

std::size_t BraggStackBandstructure::LineSet::multiplicity(std::size_t line) const
{
...
int BraggStackBandstructure::LineSet::lineType(std::size_t line) const
{
    ...
    if(std::abs(mode.first->x_) < 1e-8 && std::abs(mode.first->y_) < 1e-8 && std::abs(l_) < 1e-8)
    {
        // TE+TM modes overlap when the transverse component of the wavevector is zero.
        return 3;
    }
...
std::string BraggStackBandstructure::LineSet::lineTitle(std::size_t line) const
{
