...
// eigenvalue target
auto& jdShiftPrecond = Base::register_argument<double>(
    '\0', "jd_prec_shift", "Shift to compute the preconditioner",
    false, 1.0);
...
void runWithDimension() {
    ...
    if (useDivDGMax) {
        ...
        config.jdmax_precond_shift_ = jdShiftPrecond.getValue();
