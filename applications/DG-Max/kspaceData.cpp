...
NumericalVector& KspaceData::nextPoint()
{
...
void KspaceData::setFunctionValues(std::vector< NumericalVector >& functionValues)
{
...
void KspaceData::getIntegral ( double omega, LinearAlgebra::NumericalVector& result )
{
    ...
    for(int i=0;i<elements_.size();++i){
	    ...
	    std::vector<NumericalVector> kpointsWithHigherOmega,kpointsWithLowerOmega,fAtHigherPoints,fAtLowerPoints;
	    std::vector<double> omegaAtHigherPoints,omegaAtLowerPoints;
	    NumericalVector deltakLocal[3],reciprocalk[3],localFunctionValue(result.size());
	    double deltaOmega[3];
		...
		NumericalVector corner[3],area;
		for(int j=0;j<3;++j){
		    corner[j]=(omegaAtHigherPoints[0]-omega)/(omegaAtHigherPoints[0]-omegaAtLowerPoints[j])*kpointsWithLowerOmega[j]+
			      (omega-omegaAtLowerPoints[j])/(omegaAtHigherPoints[0]-omegaAtLowerPoints[j])*kpointsWithHigherOmega[0];
		}
		OuterProduct(corner[1]-corner[0],corner[2]-corner[0],area);
		
		for(int j=0;j<3;++j){
		    localFunctionValue+=(omegaAtHigherPoints[0]-omega)/(omegaAtHigherPoints[0]-omegaAtLowerPoints[j])*fAtLowerPoints[j]+
					(omega-omegaAtLowerPoints[j])/(omegaAtHigherPoints[0]-omegaAtLowerPoints[j])*fAtHigherPoints[0];
		}
		...
		NumericalVector corner[4],area,f[4];
		for(int j=0;j<2;++j){
		    corner[2*j  ]=(omegaAtHigherPoints[0]-omega)/(omegaAtHigherPoints[0]-omegaAtLowerPoints[j])*kpointsWithLowerOmega[j]+
				  (omega-omegaAtLowerPoints[j])/(omegaAtHigherPoints[0]-omegaAtLowerPoints[j])*kpointsWithHigherOmega[0];
		    corner[2*j+1]=(omegaAtHigherPoints[1]-omega)/(omegaAtHigherPoints[1]-omegaAtLowerPoints[j])*kpointsWithLowerOmega[j]+
				  (omega-omegaAtLowerPoints[j])/(omegaAtHigherPoints[1]-omegaAtLowerPoints[j])*kpointsWithHigherOmega[1];
				  
		    f[2*j  ]=(omegaAtHigherPoints[0]-omega)/(omegaAtHigherPoints[0]-omegaAtLowerPoints[j])*fAtLowerPoints[j]+
			     (omega-omegaAtLowerPoints[j])/(omegaAtHigherPoints[0]-omegaAtLowerPoints[j])*fAtHigherPoints[0];
		    f[2*j+1]=(omegaAtHigherPoints[1]-omega)/(omegaAtHigherPoints[1]-omegaAtLowerPoints[j])*fAtLowerPoints[j]+
			     (omega-omegaAtLowerPoints[j])/(omegaAtHigherPoints[1]-omegaAtLowerPoints[j])*fAtHigherPoints[1];
		}
		...
		NumericalVector corner[3],area;
		for(int j=0;j<3;++j){
		    corner[j]=(omega-omegaAtLowerPoints[0])/(omegaAtHigherPoints[j]-omegaAtLowerPoints[0])*kpointsWithHigherOmega[j]+
			      (omegaAtHigherPoints[j]-omega)/(omegaAtHigherPoints[j]-omegaAtLowerPoints[0])*kpointsWithLowerOmega[0];
		}
		OuterProduct(corner[1]-corner[0],corner[2]-corner[0],area);
		
		for(int j=0;j<3;++j){
		    localFunctionValue+=(omega-omegaAtLowerPoints[0])/(omegaAtHigherPoints[j]-omegaAtLowerPoints[0])*fAtHigherPoints[j]+
					(omegaAtHigherPoints[j]-omega)/(omegaAtHigherPoints[j]-omegaAtLowerPoints[0])*fAtLowerPoints[0];
		}
