...
//     ly_(ly),
//     lz_(lz),
//     sigma_(0.657014913905734),//sigma eigenfrequency calculated in Matlab
//     code size_(66)//N=10 in Matlab code
//     {
//         int i=0;
...
//
//                 antiSymmetricPrime_.push_back(std::make_pair(2*j,
//                 2*(n-j)+1));
//             }
//         }
// //*************creation of antiSymmetric modes*******************
...
//         copy (pAlpha_.begin(), pAlpha_.end(), std::ostream_iterator<double>
//         (cout, " ")); cout <<"*****"<<endl;
// //*************read coefficients from Matlab output files*******************
...
//
//
//         u += (pAlpha_[i]*sin(sigma_*t)*(epsilon(k, l)*oneOverNu(k,
//         l)*((k)/lx_)*sin((k*pi*x)/lx_)*cos((l*pi*y)/ly_))); u +=
//         (pPrimeAlpha_[i]*cos(sigma_*t)*(epsilon(kPrime,
//         lPrime)*oneOverNu(kPrime,
//         lPrime)*((kPrime)/lx_)*sin((kPrime*pi*x)/lx_)*cos((lPrime*pi*y)/ly_)));
//         u += -(qAlpha_[i]*sin(sigma_*t)*(2*oneOverNu(k,
//         l)*((l)/ly_)*sin((k*pi*x)/lx_)*cos((l*pi*y)/ly_))); u +=
...
//             lPrime = antiSymmetricPrime_[i].second;
//
//             v += (pAlpha_[i]*sin(sigma_*t)*(epsilon(k, l)*oneOverNu(k,
//             l)*((l)/ly_)*cos((k*pi*x)/lx_)*sin((l*pi*y)/ly_))); v +=
//             (pPrimeAlpha_[i]*cos(sigma_*t)*(epsilon(kPrime,
//             lPrime)*oneOverNu(kPrime,
//             lPrime)*((lPrime)/ly_)*cos((kPrime*pi*x)/lx_)*sin((lPrime*pi*y)/ly_)));
//             v += (qAlpha_[i]*sin(sigma_*t)*(2*oneOverNu(k,
//             l)*((k)/lx_)*cos((k*pi*x)/lx_)*sin((l*pi*y)/ly_))); v +=
//             (qPrimeAlpha_[i]*cos(sigma_*t)*(2*oneOverNu(kPrime,
...
//             lPrime = antiSymmetricPrime_[i].second;
//
//             w += -(sigma_*rAlpha_[i]*sin(sigma_*t)*(epsilon(k,
//             l)*cos((k*pi*x)/lx_)*sin((l*pi*y)/ly_))); w +=
//             (sigma_*rPrimeAlpha_[i]*cos(sigma_*t)*(epsilon(kPrime,
//             lPrime)*cos((kPrime*pi*x)/lx_)*cos((lPrime*pi*y)/ly_)));
//
//         }
...
// l)*k/lx_*sin((k*pi*x)/lx_)*cos((l*pi*y)/ly_)); 			s1 +=
...
// 			sx1 +=
// lz_/pi*sigma_*sigma_*rAlpha_[i]*cos(sigma_*t)*(epsilon(k,
// l)*k/lx_*((k*pi)/lx_)*cos((k*pi*x)/lx_)*cos((l*pi*y)/ly_));
//
// 			sx1 +=
// lz_/pi*sigma_*sigma_*rPrimeAlpha_[i]*sin(sigma_*t)*epsilon(kPrime,
// lPrime)*kPrime/lx_*((kPrime*pi*x)/lx_)*cos((kPrime*pi*x)/lx_)*cos((lPrime*pi*y)/ly_);
//
//
// 		}
...
// 			s2 +=
// lz_/pi*sigma_*sigma_*rAlpha_[i]*cos(sigma_*t)*(epsilon(k,
// l)*l/ly_*cos((k*pi*x)/lx_)*sin((l*pi*y)/ly_)); 			s2 +=
// lz_/pi*sigma_*sigma_*rPrimeAlpha_[i]*sin(sigma_*t)*(epsilon(kPrime,
// lPrime)*lPrime/ly_*cos((kPrime*pi*x)/lx_)*sin((lPrime*pi*y)/ly_));
//
//
...
class InitCond {
   ...
   public:
    InitCond(const ExactSolutionBase* init) : velocity_(init) {}

    virtual void operator()(const ElementT* element, const PointReferenceT& p,
                            LinearAlgebra::MiddleSizeVector& r) const = 0;
...
class InitCondU
    : public InitCond,
      public Integration::ElementIntegrandBase<LinearAlgebra::MiddleSizeVector,
                                               3> {
   public:
    using ReturnType = LinearAlgebra::MiddleSizeVector;
   ...
   public:
    ...
    void operator()(const ElementT* element, const PointReferenceT& p,
                    LinearAlgebra::MiddleSizeVector& r) const;
    void elementIntegrand(Base::PhysicalElement<3>& el,
                          LinearAlgebra::MiddleSizeVector& r) {
