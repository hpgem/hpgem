...
struct SparsityEstimator::Workspace {
    /// DoFs owned by the current processor. Stored as (dof, size) where dof is
    /// the first dof of an element, face, etc.
...
void SparsityEstimator::writeDoFCount(
    ...
    for (auto iter : workspace.owned) ownedSize += iter.second;
    for (auto iter : workspace.notOwned) notOwnedSize += iter.second;
...
void SparsityEstimator::computeSparsityEstimate(
    ...
    for (const Base::Face* face : rowIndexing_.getMesh()->getFacesList()) {
        if (!face->isOwnedByCurrentProcessor() ||
            !hasLocalDoFs(face, rowIndexing_.getIncludedUnknowns())) {
        ...
        for (const Base::Element* element : faceElements) {
            ...
            // face
            if (!includeFaceCoupling) continue;
            ...
            logger.assert_always(
                ...
                "Face coupling with conforming basis functions on distributed "
    ...
    for (const Base::Edge* edge : rowIndexing_.getMesh()->getEdgesList()) {
        if (!edge->isOwnedByCurrentProcessor() ||
            !hasLocalDoFs(edge, rowIndexing_.getIncludedUnknowns())) {
            continue;
        }
        workspace.clear();
    ...
    if (rowIndexing_.getMesh()->dimension() > 1) {
        for (const Base::Node* node : rowIndexing_.getMesh()->getNodesList()) {
            if (!node->isOwnedByCurrentProcessor() ||
                !hasLocalDoFs(node, rowIndexing_.getIncludedUnknowns())) {
                continue;
            }
            workspace.clear();
            for (const Base::Element* element : node->getElements()) {
                ...
                logger.assert_always(
                    element->isOwnedByCurrentProcessor(),
                    "Face coupling with conforming basis functions on "
                    "distributed meshes is not supported");
                for (const Base::Face* face : element->getFacesList()) {
                    if (!face->isInternal()) continue;
                    const Base::Element* otherElement =
                        face->getPtrOtherElement(element);
...
void SparsityEstimator::addElementDoFs(const Base::Element* element,
    ...
    {
        // For the element
        std::map<std::size_t, std::size_t>& toChange =
            workspace.getDoFs(element->isOwnedByCurrentProcessor());
    ...
    for (const Base::Face* face : element->getFacesList()) {
        std::map<std::size_t, std::size_t>& toChange =
            workspace.getDoFs(face->isOwnedByCurrentProcessor());
        for (std::size_t unknown : columnIndexing_.getIncludedUnknowns()) {
            const std::size_t offset =
    ...
    for (const Base::Edge* edge : element->getEdgesList()) {
        std::map<std::size_t, std::size_t>& toChange =
            workspace.getDoFs(edge->isOwnedByCurrentProcessor());
        for (std::size_t unknown : columnIndexing_.getIncludedUnknowns()) {
            const std::size_t offset =
    ...
    if (rowIndexing_.getMesh()->dimension() > 1) {
        ...
        for (const Base::Node* node : element->getNodesList()) {
            std::map<std::size_t, std::size_t>& toChange =
                workspace.getDoFs(node->isOwnedByCurrentProcessor());
            for (std::size_t unknown : columnIndexing_.getIncludedUnknowns()) {
                const std::size_t offset =
