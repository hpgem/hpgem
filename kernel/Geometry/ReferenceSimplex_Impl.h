...
namespace Geometry
{
    ...
    template<std::size_t DIM>
    ReferenceSimplex<DIM>::ReferenceSimplex(const ReferenceGeometryType& geoT, std::string name)
        : ReferenceGeometry(geoT, name)
        , center_ (createCenter<DIM>())
    {
        ...
        for(std::size_t i = 0; i <= DIM; ++i)
        {
            ...
            if(i > 0)
                this->points_[i].setCoordinate(i-1, 1);
        }
    }

    template<std::size_t DIM>
    LinearAlgebra::SmallVector<DIM+1> ReferenceSimplex<DIM>::baryCentricCoordinates(
