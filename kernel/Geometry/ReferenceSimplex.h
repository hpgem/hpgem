...
namespace Geometry
{
    ...
    public:
        ...
        LinearAlgebra::SmallVector<DIM+1> baryCentricCoordinates(const PointReference<DIM>& p) const;

        template<std::size_t DIM1>
        friend std::ostream& operator<<(std::ostream& os, const ReferenceSimplex<DIM1>& simplex);
        ...
        std::size_t getNumberOfNodes() const final
        {
            return DIM + 1;
        }

        const PointReferenceBase& getReferenceNodeCoordinate(const std::size_t& localIndex) const final
        {
            logger.assert_debug(localIndex >= 0 && localIndex < getNumberOfNodes(),
                    "Local index % should be in the range [0, %]", localIndex, getNumberOfNodes());
            return points_[localIndex];
        }

        const PointReference<DIM>& getPoints()
        {
            return points_;
        }
    ...
    template<std::size_t DIM>
    std::ostream& operator<<(std::ostream& os, const ReferenceSimplex<DIM>& simplex)
    {
        os << simplex.name_ << " =( ";
