...
class SingleElementWriter {
   public:
    ...
    /// written.
    virtual void writeOutput(const Base::Element*,
...
/// Then open gnuplot and plot with the command: plot "output.dat" every ::3
/// (1D) or splot "output.dat" (for 2D)
template <std::size_t DIM>
class GNUPlotDiscontinuousSolutionWriter {
   public:
    /// Constructor: Initialise the output stream and write the header.
    GNUPlotDiscontinuousSolutionWriter(std::ostream& output,
    ...
    GNUPlotDiscontinuousSolutionWriter(
        const GNUPlotDiscontinuousSolutionWriter& other) = delete;
    ...
    /// automatically.
    ~GNUPlotDiscontinuousSolutionWriter() { output_.flush(); }
