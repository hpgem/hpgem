...
//
//------------------------------------------------------------------------------
// Implementation part of LevelTree class.
// This file will be included in that class definition.

#ifndef LevelTree_Impl_hpp
#define LevelTree_Impl_hpp
...
{
    template <class V>
    LevelTree<V>::LevelTree() : noRootEntries_(0), 
            minLevel_(0), maxLevel_(0), activeLevel_(-1), 
            coarsestEntriesSet_(false)
        {}
...
//             std::cout << "-------------LevelTree is deleted.\n\n";
    ...
    typename LevelTree<V>::iterator LevelTree<V>::beginPostOrder()
        {   
            ...
            fci.ptr_=entries_.begin();
            for (int i=0; i<fci->getNumSiblings()-1; ++i, ++(fci.ptr_));  // the last brother
            fci.last_ = fci.ptr_;
